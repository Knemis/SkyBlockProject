...
public class FlagGUIManager {
    ...
    private void initializeFlagLayout() {
        flagLayout.clear();
        // ... (initializeFlagLayout içeriği bir önceki cevaptaki gibi kalacak, 22 bayrak tanımı)
    ...
    public void openFlagsGUI(Player player) {
        ...
        // === GEÇİCİ TEST İÇİN SABİT BOYUT ===
        int guiSize = 54; // 6 sıra, tüm slotların geçerli olması için.
        plugin.getLogger().info("[FlagGUIManager DEBUG] Kullanılan sabit GUI Boyutu: " + guiSize);
        // === ESKİ DİNAMİK BOYUT HESAPLAMASI (ŞİMDİLİK YORUMDA) ===
        // int numRows = Math.max(3, Math.min(6, ((flagLayout.size() + 8) / 9) + 1));
        // int guiSize = numRows * 9;
        // plugin.getLogger().info("[FlagGUIManager DEBUG] Hesaplanan guiSize: " + guiSize + ", numRows: " + numRows);
        ...
        // Ayırıcıları yerleştir (54 slotluk GUI'ye göre ayarlandı)
        for (int i = 0; i < 9; i++) gui.setItem(i, separator.clone()); // İlk satır
        for (int i = 18; i < 27; i+= (i==18 || i==26 ? 1 : 8)) { // 3. satırın başı ve sonu, ve aralarda
            if(i==18 || i==26) gui.setItem(i, separator.clone());
        }
        for (int i = 36; i < 45; i+= (i==36 || i==44 ? 1 : 8)) { // 5. satırın başı ve sonu
            if(i==36 || i==44) gui.setItem(i, separator.clone());
        }
        ...
        for (FlagUIData flagData : flagLayout) {
            ...
            // Bu logu her bayrak için görmek GUI açılışını yavaşlatabilir, gerekirse açın
            // plugin.getLogger().info("[FlagGUIManager DEBUG] İşlenen bayrak: " + flagData.baseDisplayName + ", Slot: " + flagData.slot);
            ...
            if (flagData.slot >= 0 && flagData.slot < gui.getSize()) {
                ...
                // Bu uyarıyı sadece detaylı loglama açıksa gösterelim
