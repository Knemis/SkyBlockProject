...
@NoArgsConstructor
public class ChatCommand<T extends com.knemis.skyblock.skyblockcoreproject.teams.database.Team, U extends com.knemis.skyblock.skyblockcoreproject.teams.database.IridiumUser<T>> extends Command<T, U> { // TODO: Update Team and IridiumUser to actual classes
    ...
    public boolean execute(U user, T team, String[] args, IridiumTeams<T, U> iridiumTeams) {
        ...
        // Optional<ChatType> chatType = iridiumTeams.getChatTypes().stream() // TODO: Uncomment when getChatTypes is available
                // .filter(type -> type.getAliases().stream().anyMatch(s -> s.equalsIgnoreCase(args[0])))
                // .findFirst();
        // if (!chatType.isPresent()) {
            // player.sendMessage(StringUtils.color(iridiumTeams.getMessages().unknownChatType // TODO: Replace StringUtils.color
        ...
        // player.sendMessage(StringUtils.color(iridiumTeams.getMessages().setChatType // TODO: Replace StringUtils.color
                // .replace("%prefix%", iridiumTeams.getConfiguration().prefix)
                // .replace("%type%", chat))
        // );
    ...
    public List<String> onTabComplete(CommandSender commandSender, String[] args, IridiumTeams<T, U> iridiumTeams) {
        // return iridiumTeams.getChatTypes().stream() // TODO: Uncomment when getChatTypes is available
