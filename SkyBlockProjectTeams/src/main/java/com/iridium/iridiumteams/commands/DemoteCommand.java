...
@NoArgsConstructor
public class DemoteCommand<T extends Team, U extends keviinUser<T>> extends Command<T, U> {
    ...
    public boolean execute(U user, T team, String[] args, keviinTeams<T, U> keviinTeams) {
        ...
        if (!DoesRankExist(nextRank, keviinTeams) || IsHigherRank(targetUser, user) || !keviinTeams.getTeamManager().getTeamPermission(team, user, PermissionType.DEMOTE)) {
            player.sendMessage(StringUtils.color(keviinTeams.getMessages().cannotDemoteUser
                    .replace("%prefix%", keviinTeams.getConfiguration().prefix)
            ));
            return false;
        }
        ...
        for (U member : keviinTeams.getTeamManager().getTeamMembers(team)) {
            ...
            if (teamMember != null) {
                if (teamMember.equals(player)) {
                    teamMember.sendMessage(StringUtils.color(keviinTeams.getMessages().demotedPlayer
                            .replace("%player%", targetUser.getName())
                            .replace("%rank%", keviinTeams.getUserRanks().get(nextRank).name)
                            .replace("%prefix%", keviinTeams.getConfiguration().prefix)
                    ));
                } else {
                    teamMember.sendMessage(StringUtils.color(keviinTeams.getMessages().userDemotedPlayer
