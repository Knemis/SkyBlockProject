...
@NoArgsConstructor
public class DepositCommand<T extends Team, U extends SkyBlockProjectTeamsUser<T>> extends Command<T, U> {
    ...
    public boolean execute(U user, T team, String[] args, SkyBlockProjectTeams<T, U> SkyBlockProjectTeams) {
        ...
        try {
            ...
            BankResponse bankResponse = bankItem.get().deposit(player, Double.parseDouble(args[1]), teamBank, SkyBlockProjectTeams);

            player.sendMessage(StringUtils.color((bankResponse.isSuccess() ? SkyBlockProjectTeams.getMessages().bankDeposited : SkyBlockProjectTeams.getMessages().insufficientFundsToDeposit)
