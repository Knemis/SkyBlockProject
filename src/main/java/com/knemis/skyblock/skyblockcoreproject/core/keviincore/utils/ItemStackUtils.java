...
//        Removed until https://github.com/CryptoMorin/XSeries/issues/266 is fixed
        if (item.material == XMaterial.PLAYER_HEAD && item.skullData != null && !item.skullData.isEmpty() && !SkyBlockSecondCore.isTesting()) {
    ...
    private static ItemStack setHeadData(String headData, ItemStack itemStack) {
        if (SkyBlockSecondCore.isTesting()) return itemStack;
