...
public class IslandDataHandler {
    ...
    public void loadIslandsFromConfig() {
        ...
        for (String uuidString : islandsSection.getKeys(false)) {
            ...
            if (worldName == null || worldName.isEmpty()) {
                plugin.getLogger().severe("CRITICAL LOAD ISSUE: World name for island (Owner: " + uuidString + ") is missing in islands.yml. Skipping this island. Its data may be lost if not manually corrected in the YML.");
                continue;
            }
            ...
            if (islandWorld == null) {
                ...
                if (this.skyblockWorld != null && worldName.equals(this.skyblockWorld.getName())) {
                    ...
                    plugin.getLogger().severe("CRITICAL LOAD ISSUE: World '" + worldName + "' for island (Owner: " + uuidString + ") not found or not loaded. Skipping this island. Its data may be lost if not manually corrected or if the world remains unavailable.");
                    continue;
                }
            }
            ...
            // Persist default regionId if it was generated by the constructor
            String loadedRegionId = islandsConfig.getString(path + "regionId"); // This is the 'regionId' variable from earlier
            String actualIslandRegionId = island.getRegionId();

            if (loadedRegionId == null || loadedRegionId.isEmpty()) {
                // This means the Island constructor generated a default regionId
                plugin.getLogger().info("Island (Owner: " + uuidString + ", Name: " + island.getIslandName() + ") was missing 'regionId' in islands.yml. Defaulting to '" + actualIslandRegionId + "' and marking for save.");
                islandsConfig.set(path + "regionId", actualIslandRegionId); // Write the default back to the config object
                dataChangedSinceLastSave = true; // Ensure this change gets saved later
            }
    ...
    private void writeIslandToConfigInternal(Island island) {
        ...
        String currentRegionId = island.getRegionId(); // Should always be valid due to constructor
        if (currentRegionId == null || currentRegionId.isEmpty()) {
            // This case should ideally not be reached if Island object is constructed correctly.
            plugin.getLogger().severe("UNEXPECTED CRITICAL DATA ISSUE: regionId for island (Owner: " + uuidString + ", IslandName: " + island.getIslandName() + ") is NULL or EMPTY despite constructor safeguards. Saving as is.");
        }
        islandsConfig.set(path + "regionId", currentRegionId); // Save the (hopefully valid) regionId
        ...
        if (baseLoc != null && baseLoc.getWorld() != null) {
            ...
            plugin.getLogger().severe("CRITICAL DATA LOSS: Base location or world for island (Owner: " + uuidString + ", IslandName: " + island.getIslandName() + ") is NULL. Location data WILL NOT BE SAVED.");
        }
